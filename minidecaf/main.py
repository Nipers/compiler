...
from .ir import IREmitter, typeCheck, GenerateName
from .ir.irgen import StackIRGen
from .ir.NameManager import NameManager
...
def main(argv):
    try:
        ...
        nameInfo = GenerateNameInfo(tree)
        typeInfo = CheckType(tree, nameInfo)
        ir = GenerateIR(tree, nameInfo, typeInfo)
        # print(ir) 目前的问题是把IR指令直接输出到汇编文件中
        GenerateAsm(ir, args.outfile)
        return 0
